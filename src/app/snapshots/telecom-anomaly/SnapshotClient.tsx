...
import snapshotData from "@/lib/telecom-signal-history-2025-08-08.json";
import {
...
export default function SnapshotClient() {
  ...
  useEffect(() => {
    const getAnalysis = async () => {
      setLoading(true);
      try {
        const output = await analyzeSignalHistory({ actionLogs: snapshotData.logs });
        setResult(output);
      } catch (error) {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>{snapshotData.description}</CardDescription>
        </CardHeader>
        <CardContent>
