...
if (isBrowser() && firebasePublicConfig.projectId) {
 logTelemetryEvent('firebase:init_start', { metadata: { config: firebasePublicConfig.projectId ? 'present' : 'missing' } });
    app = getApps().length ? getApp() : initializeApp(firebasePublicConfig);
    firestore = getFirestore(app);
    auth = getAuth(app);
    enableNetwork(firestore).catch(() => {
 logTelemetryEvent('firebase:init_success', { metadata: { projectId: firebasePublicConfig.projectId } });
...
logTelemetryEvent('firebase:init_skipped', { metadata: { isBrowser: isBrowser(), hasProjectId: !!firebasePublicConfig.projectId } });
